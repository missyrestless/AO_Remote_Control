...
// Mod: Channel for chat control (currently power on/off only).
integer chatControlChannel = 2440;
string chatControlString = "toggle_ZHAO-II-MB";
// Use "toggle_ZHAO-II-MB" on the given channel above to control the ZHAO.
...
default {
    ...
    listen( integer _channel, string _name, key _id, string _message) {
        ...
        // Mod: Check if we got a power toggle request_
        if ( _channel == chatControlChannel && _id == Owner && _message == "toggle_ZHAO-II-MB" ) {
            toggleAO();
        }

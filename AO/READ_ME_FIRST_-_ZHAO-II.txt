...
* Support for remote control commands sent via chat and gestures on a private channel

* All animations, gestures, notecards, and scripts distributed with full permissions

* All scripts optimized with LSL-PyOptimizer to reduce memory usage and improve performance

* Automatic poseball detection ('Seamless sit' mod by Moeka Kohime): When sitting, ZHAO tries to automatically determine if you're sitting on a poseball, which should make it unneccesary to turn off the AO in many cases.

* SIT ANYWHERE: Allows your avatar to sit onthe ground where you are standing by playing a (= the currently selected) 'GroundSit' animation instead of standing animations.
...
If you bought this Animation Overrider (AO) pre-loaded with animations, and you need help with the animations, please do not IM Missy Restless with questions. I (Missy) only make the AO (which is a free product), I do not make or sell any animations. The AO is a separate product from the animations that are contained within it. Some animation creators use my AO to package and sell their animations. That's why it shows me as the creator of the AO.

If you need help with the animations, you need to contact the person you bought the ZHAO-II from. To find out who to contact, try the following:

* Right click on the AO and select 'Edit'. Click on 'More', go to the 'Contents' tab, right-click on an animation and select 'Properties'. That will tell you who created the animations you bought.
...
If you bought the ZHAO-II pre-loaded with animations, then it's already been configured by the person who set it up. To use it:

* Right-click the ZHAO-II in your inventory and select 'Wear'. It should attach itself to some point on your HUD (default is bottom right)

BUTTON OVERVIEW:  􀀃

* The 'Power' button turns ZHAO-II on and off. Green means on, red means off. When ZHAO-II is off, it won't override your animations.
====
* This version of the ZHAO-II MB is based on the MONO VM so please be sure you use a client with the capability to recompile the scripts to MONO instead of LSL2.
====
* You can control the power button also via a chat line or a gesture. Easily type /2440toggle_ZHAO-II-MB into the chat to toggle the power button remotely.
* The 'Menu' button lets you customize the behavior of your ZHAO-II. The options are:
...
This section is for people familiar with the old ZHAO. 
